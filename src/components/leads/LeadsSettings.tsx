  ...
  const handleToggleModule = async (enabled: boolean) => {
    ...
    try {
      const { error } = await supabase
        .from('app_settings')
        .upsert({ setting_key: 'leads_module_enabled', setting_value: enabled, updated_by: user.id, updated_at: new Date().toISOString(), school_id: schoolId } as any, { onConflict: 'setting_key' });
